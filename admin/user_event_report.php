...
if ($report_type == 1){
	...
	$list = $services->getUserEventServices($_POST['event_id']);
} elseif ($report_type == 3){
	$title = "Event Type Wise";
	$list = $event->getEventTypeWise($_POST['event_type_id']);
} elseif ($report_type == 4){
	$title = "User Wise";
	$list = $event->getUserEvents($_POST['user_id']);
} elseif ($report_type == 5){
	$title = "Vendor Wise";
	$list = $event->getUserEvents($_POST['vendor_id'],'vendor');
} elseif ($report_type == 6){
	$title = "Location Wise";
	$list = $event->getLocWiseUserEvents($_POST['country_id'], $_POST['state_id'], $_POST['city_id']);
}elseif ($report_type == 7){
	$title = "Date Wise Event";
	$list = $event->getDateWiseEvents($_POST['date']);
}
